...
# Contributing to centinelas-pr

Thanks for helping improve `centinelas-pr` — the universal online intelligence monitor and routing engine in the Puerto Rico
...
| Template drift | Run `python3 tools/render_federation_templates.py --repo centinelas-pr --repo-root /path/to/centinelas-pr --check` from a local Hub checkout. |
