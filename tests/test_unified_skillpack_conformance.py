...
class UnifiedSkillpackConformanceTests(unittest.TestCase):
    ...
    def test_dispatch_metadata_is_complete(self) -> None:
        manifest = json.loads(
            (ROOT / ".claude/skillpacks/MANIFEST.json").read_text(encoding="utf-8")
        )
    ...
    def test_compatibility_targets_resolve(self) -> None:
        ...
        skill = (ROOT / ".claude/skillpacks/SKILL.md").read_text(encoding="utf-8")
        for entry in ledger["entries"]:
            target = entry["unified_target"].split("#", 1)[1]
            self.assertIn(f"{{#{target}}}", skill, entry["capability_id"])
