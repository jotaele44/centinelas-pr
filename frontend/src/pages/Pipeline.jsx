  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      <section className="rounded-2xl border bg-gradient-to-b from-primary/5 to-background p-6">
        <p className="text-sm font-semibold uppercase tracking-wide text-primary">{t("Admisión universal")}</p>
        ...
        <p className="mt-3 max-w-3xl text-muted-foreground">
          {t("Contenido en línea clasificado en seis dominios y despachado a los repositorios de la federación correspondientes. Cada item también se registra en thehub-pr sin importar la clasificación.")}
        </p>
        <div className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground">
          <span>{t("Clasificados:")} <strong className="text-foreground">{status.classified ?? "…"}</strong></span>
          <span>{t("Despachados:")} <strong className="text-foreground">{status.dispatched ?? "…"}</strong></span>
          <span>{t("En cola:")} <strong className="text-foreground">{status.queue ?? "…"}</strong></span>
        </div>
      </section>
      ...
      {!reachable && !loading ? (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base"><Server className="h-4 w-4" /> {t("Backend no accesible")}</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            {t("Inicia el API de admisión desde la raíz del repositorio:")}
      ...
      {loading ? (
        <p className="rounded-xl border p-6 text-muted-foreground">{t("Cargando items…")}</p>
      ) : items.length === 0 ? (
        <p className="rounded-xl border p-6 text-muted-foreground">
          {t("No hay items clasificados")}{domain ? ` · ${DOMAIN_META[domain].label}` : ""}. {t("Ejecuta el pipeline con")}{" "}
          <code className="font-mono text-xs">centinelas run</code> {t("para poblarlo.")}
        </p>
      ) : (
