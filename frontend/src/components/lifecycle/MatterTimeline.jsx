  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <h3 className="font-semibold text-foreground">Línea de vida del asunto</h3>
          <p className="text-sm text-muted-foreground">Centinelas detecta intención; MoneySweep verifica ejecución oficial.</p>
        </div>
        <span className="text-xs text-muted-foreground">Etapa {active.order}/6</span>
      </div>
        ...
        {MATTER_LIFECYCLE_STAGES.map((stage) => {
          ...
          return (
            <div key={stage.key} className={`rounded-lg border p-3 ${isActive ? "border-primary bg-primary/10" : isReached ? "bg-muted/60" : "bg-background"}`}>
              <div className="text-xs font-semibold text-muted-foreground">{stage.app}</div>
              <div className="mt-1 text-sm font-medium text-foreground">{stage.label}</div>
            </div>
          );
        })}
      </div>
