...
function PhaseBadge({ phase }) {
  const tone = {
    ...
    NEXT: "border-emerald-500/50 text-emerald-600",
    QUEUED: "border-blue-500/50 text-blue-600",
    ...
    <section className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(18rem,.8fr)]" aria-label="Program timeline and upcoming work">
      <Card>
        <CardHeader className="border-b pb-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="font-mono text-[10px] font-bold tracking-[0.12em] text-muted-foreground">PROGRAM ACTIVITY</p>
              <CardTitle className="mt-1 text-xl">Timeline</CardTitle>
            </div>
          ...
          <div className="grid gap-3 border-b p-4">
            <div className="flex gap-2 overflow-x-auto pb-1">
              {["ALL", "NOW", "NEXT", "QUEUED", "BLOCKED"].map((value) => (
                <button key={value} type="button" onClick={() => setFilter(value)} className={`min-h-9 rounded-lg border px-3 font-mono text-[11px] font-semibold ${filter === value ? "border-primary/60 bg-primary/10 text-foreground" : "text-muted-foreground"}`}>
                  {value}
                </button>
              ))}
              <button type="button" onClick={() => setSort((value) => value === "priority" ? "name" : "priority")} className="ml-auto inline-flex min-h-9 items-center gap-2 rounded-lg border px-3 font-mono text-[11px] font-semibold text-muted-foreground">
            ...
            <label className="flex items-center gap-2 rounded-lg border bg-background px-3">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search activity, function, or state" className="min-h-11 w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
            </label>
          </div>
      ...
      <Card>
        <CardHeader className="border-b pb-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="font-mono text-[10px] font-bold tracking-[0.12em] text-muted-foreground">NEXT QUEUE</p>
              <CardTitle className="mt-1 text-xl">Upcoming work</CardTitle>
            </div>
