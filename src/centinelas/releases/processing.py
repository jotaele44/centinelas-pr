...
def repair_pdf_in_sandbox(
    ...
    if repairer is None:
        raise ValueError(
            "corrupt PDF: missing EOF marker and no sandbox repairer supplied"
        )
    repaired = repairer(data)
    if not repaired.startswith(PDF_MAGIC) or b"%%EOF" not in repaired[-2048:]:
        raise ValueError(
            "sandbox repair output failed PDF signature validation"
        )
...
def _split_pdf_pages(data: bytes) -> list[bytes]:
    starts = [
        match.start()
        for match in re.finditer(rb"/Type\s*/Page\b", data)
    ]
...
def _flags(page_bytes: bytes, text: str) -> tuple[str, ...]:
    ...
    if any(
        token in lowered
        for token in ("latitude", "longitude", "map", "chart", "legend")
    ):
        flags.add("MAP_OR_CHART")
    if any(
        token in lowered
        for token in ("table", "column", "subtotal", "total")
    ):
        flags.add("TABLE")
...
def redaction_region_count(page_bytes: bytes) -> int:
    black_fills = len(
        re.findall(
            rb"(?:0\s+0\s+0\s+rg|0\s+g).*?re\s+f",
            page_bytes,
            flags=re.S,
        )
    )
...
def extract_embedded_attachments(
    data: bytes,
    out_dir: Path | None = None,
) -> tuple[str, ...]:
    matches = list(re.finditer(rb"/Type\s*/EmbeddedFile", data))
    names = tuple(
        f"attachment-{index + 1:04d}.bin"
    ...
    *,
    ...
    for page_number, page_bytes in enumerate(page_blobs, start=1):
        ...
        if needs_ocr:
            tasks.append(
                OcrTask(
                    content_sha256,
                    page_number,
                    "LOW_NATIVE_TEXT_CONFIDENCE",
                    100,
                )
            )
            if ocr is not None:
                _ocr_text, ocr_confidence = ocr(
                    page_bytes,
                    page_number,
                )
        page = PageAnalysis(
        ...
        if needs_ocr and (
            ...
            or ocr_confidence < low_confidence_threshold
        ):
    ...
    result = ProcessingResult(
        ...
        text_layer_present=any(
            page.native_text_confidence > 0
            for page in pages
        ),
