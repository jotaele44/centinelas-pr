...
SOURCE_REGISTRY: dict[str, SourceDefinition] = {
    "nara_ndc": SourceDefinition(
        "nara_ndc",
        "NARA",
        "https://www.archives.gov",
        "/declassification/ndc/releases",
    ),
    "nara_catalog": SourceDefinition(
        "nara_catalog",
        "NARA",
        "https://catalog.archives.gov",
        "/api/v1/records",
    ),
    "cia_reading_room": SourceDefinition(
        "cia_reading_room",
        "CIA",
        "https://www.cia.gov",
        "/readingroom/search/site",
    ),
    "nsa_releases": SourceDefinition(
        "nsa_releases",
        "NSA",
        "https://www.nsa.gov",
        "/Helpful-Links/NSA-FOIA/Declassification-Transparency-Initiatives/",
    ),
    "dia_reading_room": SourceDefinition(
        "dia_reading_room",
        "DIA",
        "https://www.dia.mil",
        "/FOIA/FOIA-Electronic-Reading-Room/",
    ),
    "nhhc": SourceDefinition(
        "nhhc",
        "NHHC",
        "https://www.history.navy.mil",
        "/research/archives/digital-exhibits-highlights.html",
    ),
    "doe_aec": SourceDefinition(
        "doe_aec",
        "DOE",
        "https://www.energy.gov",
        "/nnsa/foia-reading-room",
    ),
    "air_force_blue_book": SourceDefinition(
        "air_force_blue_book",
        "USAF",
        "https://www.archives.gov",
        "/research/military/air-force/ufos",
    ),
}
...
class PublicReleaseAdapter:
    def __init__(
        ...
        *,
        ...
        self.cache = HttpCache(
            state_dir / "http-cache" / definition.adapter_id
        )
    ...
    def _enforce_policy(self) -> None:
        if not self.explicitly_enabled:
            raise AdapterPolicyError(
                f"{self.adapter_id} is disabled; explicit source enablement is required"
            )
        if self.definition.robots_review_required and not self.robots_approved:
            raise AdapterPolicyError(
                f"{self.adapter_id} robots review has not been approved"
            )
        if self.definition.terms_review_required and not self.terms_approved:
            raise AdapterPolicyError(
                f"{self.adapter_id} terms review has not been approved"
            )

    def _load_checkpoint(self) -> tuple[int, bool]:
        if not self.checkpoint_path.exists():
            return 1, False
        state = json.loads(self.checkpoint_path.read_text())
        return int(state.get("next_page", 1)), bool(state.get("completed"))
    ...
    def _save_checkpoint(self, next_page: int, *, completed: bool) -> None:
        ...
        temp.write_text(
            json.dumps(
                {"next_page": next_page, "completed": completed},
                sort_keys=True,
            )
            + "\n"
        )
    ...
    def _request(self, url: str, page: int) -> bytes:
        ...
        if cached is not None:
            ...
            status, _headers, body = self.transport(
                ...
                {
                    "User-Agent": (
                        "centinelas-pr-public-archive-monitor/1.0"
                    )
                },
            )
            cache_hit = False
        ...
        if status != 200:
            raise RuntimeError(
                f"{self.adapter_id} HTTP {status} for page {page}"
            )
        return body

    def enumerate_pages(
        self,
        parse_page: Callable[[bytes], tuple[list[dict[str, Any]], bool]],
    ) -> Iterable[dict[str, Any]]:
        self._enforce_policy()
        page, completed = self._load_checkpoint()
        if completed:
            return
        while True:
            query = urlencode(
                {
                    self.definition.page_parameter: page,
                    self.definition.page_size_parameter: (
                        self.definition.page_size
                    ),
                }
            )
            url = (
                f"{self.definition.base_url}{self.definition.query_path}"
                f"?{query}"
            )
