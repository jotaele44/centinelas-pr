...
| D2 | Data reality | **2** | Declares `production_status: PRODUCTION` with `data/` at 304 KB, 2 of 3 data files synthetic-flagged |
| D3 | UI craft | **3** | 16 pages, 4.6k LOC, 35 `aria-*`, 10 `role=` — good markup, but **0 empty-state components** and no `ErrorBoundary` |
...
The engineering is sound for its size. The issue is that the login screen presents a
credential check that does not exist — and this repo declares itself `PRODUCTION`.
...
| Legislative pages (Signals, Matters, Handoff, Entities, SignalsTable) | fully interactive, but every write lands in browser storage only |

**SCAFFOLD**

| Item | Why |
|---|---|
| Empty/error UX | 0 empty-state components and 0 `ErrorBoundary` across 16 pages — a failed fetch has no designed surface |
...
| Monitor, Sources, Handoff | mixed | partial | **Functional** |
...
| 2 | Reconcile `production_status: PRODUCTION` with 304 KB of data, 2/3 files synthetic-flagged | **S** | Either the status or the data needs to move. Right now the manifest overstates the node. |
| 3 | Add empty-state and error-state components | **M** | 0 empty states and 0 `ErrorBoundary` across 16 pages. `skywatcher-pr/frontend/src/components/skywatcher/EmptyState.jsx` is a ready pattern to copy. |
